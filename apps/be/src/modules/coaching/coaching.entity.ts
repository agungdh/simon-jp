...
export const coachings = pgTable(
  ...
  (table) => ({
    uuidIdx: index("coachings_uuid_idx").using("hash", table.uuid),
    tanggalIdx: index("coachings_tanggal_idx").on(table.tanggalPelaksanaan),
  }),
);

export const coachingConfig: TrainingConfig = {
  name: "coaching",
  table: coachings,
  owner: true,
