...
export const mentorings = pgTable(
  "mentorings",
  {
    id: id(),
    uuid: uuid(),
  ...
  (table) => ({
    uuidIdx: index("mentorings_uuid_idx").using("hash", table.uuid),
    tanggalIdx: index("mentorings_tanggal_idx").on(table.tanggalPelaksanaan),
  }),
);

export const mentoringConfig: TrainingConfig = {
  name: "mentoring",
  table: mentorings,
  owner: true,
